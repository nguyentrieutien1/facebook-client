...
export default {
  ...
  methods: {
    ...
    async handleSendMessenger() {
      const data = await messengerAction.handleSendMessenger(
        this.textC,
        this.friendId,
        this.myId
      );
    ...
    handleSeenMess(e) {
      messengerAction
        ...
        .then((data) => {
          messengerAction.countMess(this.myId);
        });
    },
  },
  created() {
    ...
    socket.on("handle_entering_server", ({ value }) => {
      ...
      if (value) {
        messElement.setAttribute(
          "placeholder",
          "Đối tượng đang nhập tin nhắn, vui lòng đợi"
        );
  ...
  <div class="col-lg-5 message">
    <div class="card">
      <div class="card-header">
        <div class="card-info">
          ...
          <div class="card-heade-name">
            <h4>
              {{
                myId === accountDetail.account?.id
                  ? "Only Me"
                  : accountDetail.account?.username
              }}
            </h4>
          </div>
        </div>
      ...
      <div class="card-body card-content-messenger">
        <div
          v-for="(messager, index) in messengerList"
          :class="[
            messager.username === this.accountDetail.account.username &&
            myId !== accountDetail.account?.id
              ? 'left'
              : 'right',
            'card-body-mess',
        ...
        >
          <img
            :src="messager.avatar"
            alt=""
            v-if="messager.username === this.accountDetail.account.username"
          />
      ...
      <div>
        <form @submit.prevent="handleSendMessenger" class="card-footer">
          <input
            @focus="handleSeenMess"
            @keyup="handleEntering"
            class="form-control"
            type="text"
            id="mess"
...
.input::placeholder {
  font-size: 1em;
  animation: input 0.8s linear infinite alternate;
}
